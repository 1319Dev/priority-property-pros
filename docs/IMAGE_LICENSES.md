...
The branded homepage banner is **original first-party Priority Property Pros
...
- Below-the-fold images use `loading="lazy"`. The homepage branded banner,
  homepage house sidebar, Find a Pro header, How it works header, and Become a
  Pro header are eager.
- Width / height attributes and aspect-ratio frames are set to limit CLS. The
  branded banner uses its intrinsic 3:2 ratio (`object-contain`, no max-height
  crop) so the logo and CTA bar are not clipped.
